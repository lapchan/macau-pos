...
export async function POST(req: Request, ctx: RouteContext) {
  ...
  // 1. Look up tenant + config. Unknown tenant/slug → 404 with no detail.
  ...
  // 2. Read the raw body exactly once, then hash it for signature verification.
  //    Must not re-serialize.
  ...
  // Some env loaders pass through literal backslash-n sequences; normalize them
  ...
  // 3. Parse envelope (signature is already valid — we're only parsing here).
  ...
  // 4. Dedupe on (tenant_id, event_id). Insert the audit row first; if it
  //    collides with the unique index, we've already processed this event and
  //    can short-circuit with a 200 so intellipay stops retrying.
  try {
    ...
    // Drizzle wraps pg errors — unique_violation (23505) lives on .cause.code.
    const pgCode =
  ...
  // 5. Project the event onto the payment row. Unknown event types are
  //    accepted (2xx) and logged per spec §7.3.
  ...
  // Flip the linked order on terminal states.
