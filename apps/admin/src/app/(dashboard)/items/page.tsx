...
import { products, categories, type Product } from "@/data/mock";
import {
  Search,
  Plus,
  Upload,
...
> = {
  ...
  archived: { label: "Archived", className: "bg-surface-hover text-text-tertiary" },
};
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        {selected.size > 0 && (
          <div className="flex items-center gap-2 text-sm">
            ...
            <button className="px-3 py-1.5 text-xs font-medium text-danger border border-danger/20 rounded-[var(--radius-sm)] hover:bg-danger-light transition-colors">
              Delete
            </button>
            <button
              onClick={() => setSelected(new Set())}
              className="px-3 py-1.5 text-xs font-medium text-text-secondary border border-border rounded-[var(--radius-sm)] hover:bg-surface-hover transition-colors"
            >
              Clear
            </button>
          </div>
                  ...
                  <th key={col.key} className={cn("px-4 py-3", col.right ? "text-right" : "text-left")}>
                    <button
                      ...
                      className={cn(
                        ...
                        sortKey === col.key ? "text-text-primary" : "text-text-secondary",
                        col.right && "ml-auto"
                      )}
                    >
                      {col.label}
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      ...
                      isSelected ? "bg-accent-light/30" : "hover:bg-surface-hover/50"
                    )}
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-[var(--radius-sm)] bg-surface-hover flex items-center justify-center shrink-0">
                          <Package className="h-5 w-5 text-text-tertiary" strokeWidth={1.5} />
                        </div>
                        <div className="min-w-0">
                          <p className="font-medium text-text-primary truncate">{product.name}</p>
                          <p className="text-xs text-text-tertiary truncate">
                            {product.nameCn} · {product.sku}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right font-medium tabular-nums text-text-primary">
                      MOP {product.price.toFixed(1)}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">
                      <span
                        className={cn(
                          "font-medium",
                          product.stock === 0 ? "text-danger" : product.stock < 30 ? "text-warning" : "text-text-primary"
                        )}
                      >
                        {product.stock}
                      </span>
                    ...
                    <td className="px-4 py-3">
                      <span className={cn("inline-flex px-2 py-0.5 text-xs font-medium rounded-[var(--radius-full)]", badge.className)}>
                        {badge.label}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="relative">
                        <button
                          ...
                          aria-expanded={menuOpen === product.id}
                          className="p-1 rounded-[var(--radius-sm)] text-text-tertiary hover:text-text-secondary hover:bg-surface-hover transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100"
                        >
                  ...
                  <td colSpan={7} className="px-4 py-16 text-center">
                    <div className="flex flex-col items-center">
                      <div className="h-12 w-12 rounded-[var(--radius-md)] bg-surface-hover flex items-center justify-center mb-3">
                        <Package className="h-6 w-6 text-text-tertiary" strokeWidth={1.5} />
                      </div>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-border">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1.5 rounded-[var(--radius-sm)] text-text-tertiary disabled:opacity-40"
              >
                <ChevronLeft className="h-4 w-4" />
              </button>
              <button aria-label="Page 1" aria-current="page" className="h-8 w-8 rounded-[var(--radius-sm)] text-sm font-medium bg-text-primary text-white">
                1
              </button>
              <button
                disabled
