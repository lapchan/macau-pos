...
export default function ShiftSummaryPanel({ shiftId, onClose, onEndShift, locale = "en" }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="w-full max-w-md mx-0 sm:mx-4 bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
        {/* Header */}
        ...
        <div className="px-5 py-4 space-y-4">
              ...
              <div className="space-y-2">
                {Object.entries(summary.paymentBreakdown).map(([method, total]) => (
                  <div key={method} className="flex items-center justify-between py-2 px-4 bg-gray-50 rounded-xl">
                    <span className="text-[13px] text-gray-700">{METHOD_LABELS[method] || method}</span>
          ...
          <div className="py-3 px-4 bg-blue-50 rounded-xl">
            ...
            <p className="text-[18px] font-bold text-blue-700 tabular-nums">
              MOP {summary.expectedCash.toFixed(2)}
            </p>
        ...
        <div className="sticky bottom-0 bg-white px-5 py-4 border-t border-gray-100">
          <button
            onClick={onEndShift}
            className="w-full h-[44px] rounded-xl text-[14px] font-medium text-white transition-all active:scale-[0.98]"
