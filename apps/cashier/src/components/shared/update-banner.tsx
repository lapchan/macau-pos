...
export default function UpdateBanner({ visible }: Props) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("pos-locale");
    if (saved && saved in localeNames) setLocale(saved as Locale);
  }, []);
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-[9998] flex justify-center px-3 pt-3 pointer-events-none">
      <div className="pointer-events-auto flex items-center gap-3 rounded-xl bg-blue-600 text-white px-4 py-2.5 shadow-lg max-w-md w-full">
